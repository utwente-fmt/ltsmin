...
/* Generate a filename with the given suffix (without the directory
** part).  Space to hold the name comes from malloc() and must be
...
char *suffix;
{
  ...
  basename = strrchr (lemp->filename, '/');
  if (basename != NULL)
      ++basename;
  else
      basename = lemp->filename;
  name = malloc( lemonStrlen(basename) + lemonStrlen(suffix) + 5 );
  if( name==0 ){
