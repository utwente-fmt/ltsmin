...
static data_stream_t *output_src=NULL;
static data_stream_t *output_label=NULL;
static data_stream_t *output_dest=NULL;

/*
static data_stream_t parent_seg=NULL;
static data_stream_t parent_ofs=NULL;
static data_stream_t parent_lbl=NULL;

static data_stream_t target_file=NULL;
*/
...
static ATmap ATmapCreate(data_stream_t s){
	ATmap map;
	int i;
...
int main(int argc, char*argv[]){
	...
	if (write_lts) {
		...
		output_label=(data_stream_t*)malloc(mpi_nodes*sizeof(FILE*));
		output_dest=(data_stream_t*)malloc(mpi_nodes*sizeof(FILE*));
		for(i=0;i<mpi_nodes;i++){
			sprintf(name,"src-%d-%d",i,mpi_me);
			output_src[i]=DScreate(arch_write(arch,name),SWAP_NETWORK);
			sprintf(name,"label-%d-%d",i,mpi_me);
			output_label[i]=DScreate(arch_write(arch,name),SWAP_NETWORK);
			sprintf(name,"dest-%d-%d",i,mpi_me);
			output_dest[i]=DScreate(arch_write(arch,name),SWAP_NETWORK);
			tcount[i]=0;
		}
	}
	/***************************************************/
...
*/
	...
	/***************************************************/
	if (mpi_me==0) {
		map=ATmapCreate(DScreate(arch_write(arch,"TermDB"),SWAP_NETWORK));
	} else {
...
*/
	...
	data_stream_t info=NULL;
		if (mpi_me==0){
			...
			info=DScreate(arch_write(arch,"info"),SWAP_NETWORK);
			DSwriteU32(info,31);
