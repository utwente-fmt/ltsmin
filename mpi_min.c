...
#define GET_OFS(id) (id/mpi_nodes-1)
	...
	output_dest=(data_stream_t*)RTmalloc(mpi_nodes*sizeof(FILE*));
	...
	int *temp=NULL;
	if (mpi_me==0){
		data_stream_t ds=DScreate(arch_write(arch,"TermDB"),SWAP_NETWORK);
		...
		DSclose(&ds);
		infos=DScreate(arch_write(arch,"info"),SWAP_NETWORK);
		DSwriteU32(infos,31);
