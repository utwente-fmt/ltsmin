...
static permutation_perm_t permutation_red = Perm_Shift;
...
extern void         gnn_blue (thread_ctx_t *ctx, size_t work);
...
/* YELLOW DFS */
static void
ndfs_yellow (thread_ctx_t *ctx, idx_t seed)
{
...
static inline void
nndfs_explore_state_red (thread_ctx_t *ctx, counter_t *cnt)
{
    int                 count;
    dfs_stack_enter (ctx->stack);
    increase_level (cnt);
    count = permute_all (ctx->permute, ctx->state.data, nndfs_red_handle, ctx);
    cnt->trans += count;
    cnt->explored++;
    ndfs_maybe_report(ctx->search, cnt);
}
...
static inline void
nndfs_explore_state_blue (thread_ctx_t *ctx, counter_t *cnt)
{
    int                 count;
    dfs_stack_enter (ctx->stack);
    increase_level (cnt);
...
static inline void
gnn_explore_state_red (thread_ctx_t *ctx, counter_t *cnt)
{
    int                 count;
    dfs_stack_enter (ctx->stack);
    increase_level (cnt);
    count = permute_all (ctx->permute, ctx->state.data, gnn_red_handle, ctx);
    cnt->trans += count;
    cnt->explored++;
    ndfs_maybe_report(ctx->search, cnt);
}
...
static inline void
gnn_explore_state_blue (thread_ctx_t *ctx, counter_t *cnt)
{
    int                 count;
    dfs_stack_enter (ctx->stack);
    increase_level (cnt);
    count = permute_all (ctx->permute, ctx->state.data, gnn_blue_handle, ctx);
    cnt->trans += count;
    cnt->explored++;
    ndfs_maybe_report(ctx->search, cnt);
}
...
/* BLUE DFS (of CYAN) */
void
gnn_blue (thread_ctx_t *ctx, size_t work)
{
    while ( !lb_is_stopped(lb) ) {
        raw_data_t          state_data = dfs_stack_top (ctx->stack);
        if (NULL != state_data) {
