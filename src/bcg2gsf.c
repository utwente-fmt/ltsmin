...
int main(int argc,char**argv){
	...
	ds=arch_write(gsf,"actions",NULL);
	invis=0;
	for(i=0;i<N;i++){
	...
	DSclose(&ds);
	stream_t src=arch_write(gsf,"src-0-0",NULL);
	stream_t lbl=arch_write(gsf,"label-0-0",NULL);
	stream_t dst=arch_write(gsf,"dest-0-0",NULL);
