...
struct vector_domain_shared {
    int size;
    vset_t (*set_create)(vdom_t dom,int k,int* proj,int r_k, int *r_proj);
    ...
    vrel_t (*rel_create)(vdom_t dom,int k,int * proj,int r_k,int* r_proj,int w_k,int* w_proj);
    ...
    void (*rel_add)(vrel_t rel,const int* src,const int* dst,const int* cpy);
    void (*rel_add_rw)(vrel_t rel,const int* src,const int* dst);
...
extern void vdom_set_name(vdom_t dom, int i, char* name);

extern char* vdom_get_name(vdom_t dom, int i);

/**
\brief returns whether the vset implementation separates read and write dependencies.
*/
extern int vdom_separates_rw();
