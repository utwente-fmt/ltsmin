...
static vset_t
set_create_mdd(vdom_t dom, int k, int *proj, int r_k, int *r_proj)
{
    if (r_proj == NULL) {
        r_k = k;
        r_proj = proj;
    ...
    set->p_len = r_k;
    set->p_id  = 1;
    for(int i = r_k - 1; i >= 0; i--) {
        set->proj[i] = r_proj[i];
        set->p_id    = mdd_create_node(r_proj[i], set->p_id, 0, COPY_DONT_CARE);
...
{
    vset_t result = set_create_mdd(dom, -1, NULL, -1, NULL);
...
static vrel_t
rel_create_mdd(vdom_t dom, int k, int *proj, int r_k, int *r_proj, int w_k, int *w_proj)
{

    if (r_proj == NULL && w_proj == NULL) {
        r_k = k;
        w_k = k;
        r_proj = proj;
        w_proj = proj;
...
rel_load_proj_bin(FILE* f, vdom_t dom)
{
    ...
    stream_close(&s);
    vrel_t result = rel_create_mdd(dom, p_len, NULL, r_p_len, r_proj, w_p_len, w_proj);
    return result;
}
...
static void
rel_add_mdd(vrel_t rel, const int *src, const int *dst, const int *cpy) {
...
set_least_fixpoint_mdd(vset_t dst, vset_t src, vrel_t rels[], int rel_count)
{
    ...
    for (int grp = 0; grp < rel_count; grp++) {
        proj_set[grp] = set_create_mdd(rels[grp]->dom, -1, NULL, rels[grp]->r_p_len,
                                       rels[grp]->r_proj);
