...
struct vector_set {
    ...
    MDD proj;         // "meta" for set_project (vs full vector)
};
...
set_create(vdom_t dom, int k, int *proj)
{
    ...
    if (k < 0) {
        ...
        _proj[0] = -1;
        set->proj = lddmc_cube((uint32_t*)_proj, 1);
    } else if (k == 0) {
        _proj[0] = -2;
        set->proj = lddmc_cube((uint32_t*)_proj, 1);
    } else {
        ...
        _proj[proj[k-1]+1] = -2;
        set->proj = lddmc_cube((uint32_t*)_proj, proj[k-1]+2);
    }

    lddmc_protect(&set->mdd);
    lddmc_protect(&set->proj);
...
set_enum_match(vset_t set, int p_len, int *proj, int *match, vset_element_cb cb, void *context)
{
    ...
    _proj[len-1] = -1;
    // convert array to dd
    MDD mdd_proj = lddmc_refs_push(lddmc_cube((uint32_t*)_proj, len));
    // convert match to dd
    MDD cube = lddmc_refs_push(lddmc_cube((uint32_t*)match, p_len));
...
set_project(vset_t dst, vset_t src)
{
    if (dst->proj == src->proj) {
        ...
        dst->mdd = lddmc_project(src->mdd, dst->proj);
    }
}
...
set_project_minus(vset_t dst, vset_t src, vset_t minus)
{
    if (dst->proj == src->proj) {
        ...
        LACE_ME;
        assert(src->size == dst->dom->shared.size);
        dst->mdd = lddmc_project_minus(src->mdd, dst->proj, minus->mdd);
    }
}
...
{
    LACE_ME;
    dst->mdd = lddmc_join(left->mdd, right->mdd, left->proj, right->proj);
}
...
set_load(FILE* f, vdom_t dom)
{
    ...
    if (fread(&proj, sizeof(size_t), 1, f) < 1) Abort("Invalid file format.");
