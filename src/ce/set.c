...
void SetFree(){
	Warning(1,"Freeing set structure with %d nodes and %d edges.",setnodenext,setbucketnext);
	free(setnodes);
	setnodes=NULL;
	setnodesize=0;
	setnodenext=1;
...
void SetClear(int tag){
	...
	undefined_tag=tag;
	setnodes[0].tag=tag;
	Warning(1,"Clearing set structure with %d nodes and %d edges.",setnodenext,setbucketnext);
	setnodenext=1;
	setbucketnext=0;
