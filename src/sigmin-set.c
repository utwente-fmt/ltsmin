...
static void map_lts(seg_lts_t lts){    
    ...
    Warning(debug,"counting in edges");
    ...
    for(int i=0;i<mpi_nodes;i++){
        Warning(debug,"in count from %d is %d",i,in_count[i]);
    }
    Warning(debug,"counting out edges");
    ...
    for(int i=0;i<mpi_nodes;i++){
        Warning(debug,"out count to %d is %d",i,out_count[i]);
    }
    if (mpi_me==0) Warning(debug,"scanning for worst case fanout");
    int max_local=0;
    for(uint32_t i=0;i<my_states;i++){
    ...
    Warning(debug,"local LTS info ready");
}
...
static void id_exchange_all(){
    ...
    for(uint32_t i=0;i<my_states;i++){
        ...
        if (oldid[i]<0) Fatal(1,error,"bad ID");
    }
    for(int i=0;i<mpi_nodes;i++){
...
static void inv_fini(){
    ...
    if (RTverbosity>1) Warning(info,"%d: submission finished",mpi_me);
    event_while(mpi_queue,&inv_wait);
    event_barrier_wait(barrier);
    Warning(debug,"backpropagation setup: %d sent %d received",inv_send,inv_recv);
}
...
static void sig_set(int dst_seg,int edge,int sig){
    ...
    if(len>SIG_BUFFER_SIZE) {
        Warning(debug,"using special send");
...
sig_id_t *sigmin_set_branching(event_queue_t queue,seg_lts_t lts,uint32_t tau_arg){
    ...
    while(oldcount!=newcount){
        ...
        if (mpi_me==0 && RTverbosity>1) {
            Warning(info,"computing initial signatures and marking invisible steps");
        }
        inv_init(in_state);
        event_barrier_wait(barrier);
        for(uint32_t i=0;i<my_states;i++){
        ...
        inv_fini();
        event_barrier_wait(barrier);
        if (mpi_me==0) Warning(debug,"starting forwarding phase");
        fwd_init();
        event_barrier_wait(barrier);
        ...
        while(total_new_set_count>0){
            ...
            if (mpi_me==0) Warning(debug,"sub iteration yielded %d modified sigs.",total_new_set_count);
        }
        event_barrier_wait(barrier);
        if (mpi_me==0 && RTverbosity>1) Warning(info,"%d: exchanging signatures",mpi_me);
        synch_reset();
        event_barrier_wait(barrier);
        for(uint32_t i=0;i<my_states;i++){
    ...
    if (sizeof(sig_id_t)!=sizeof(int)){
        Fatal(1,error,"size mismatch sig_id_t and int");
    }
...
sig_id_t *sigmin_set_strong(event_queue_t queue,seg_lts_t lts){
    ...
    while(oldcount!=newcount){
        if (iter) {
            if (mpi_me==0) Warning(debug,"forwarding signatures");
            id_exchange_all();
        }
        iter++;
        oldcount=newcount;
        SetClear(SET_TAG_UNDEF);
        if (mpi_me==0) Warning(debug,"computing signatures");
        for(uint32_t i=0;i<my_states;i++){
            int s=EMPTY_SET;
            for(uint32_t j=dest_begin[i];j<dest_begin[i+1];j++){
                s=SetInsert(s,dest_label[j],map[dest_seg[j]][dest_edge[j]]);
        ...
        event_barrier_wait(barrier);
        if (mpi_me==0) Warning(debug,"synchronizing signatures");
        synch_reset();
        for(uint32_t i=0;i<my_states;i++){
            if(SetGetTag(set[i])==SET_TAG_UNDEF){
                SetSetTag(set[i],SET_TAG_ASKED);
...
static void build_tau_levels(){
    ...
    if(mpi_me==0){
        ...
        SCCstartTimer(timer);
        Warning(debug,"computing tau levels");
    }
    event_barrier_wait(barrier);
    ...
    for(;;){
        ...
        Warning(debug,"scanning for states without unmarked tau edges on level %d",level_count);
        level_count++;
        level_begin[level_count]=level_begin[level_count-1];
        long long int local_found=0;
        for(uint32_t i=0;i<my_states;i++){
            Warning(debug,"state %u",i);
            if (bitset_test(level_done,i)) continue;
            int has_unmarked=0;
            Warning(debug,"scanning %u [%d-%d]",i,dest_begin[i],dest_begin[i+1]);
            for(uint32_t j=dest_begin[i];j<dest_begin[i+1];j++){
                Warning(debug,"%u --%d-> %u:%u",i,dest_label[j],dest_seg[j],dest_edge[j]);
                if (dest_label[j]==tau && !bitset_test(level_visited[dest_seg[j]],dest_edge[j])) {
                    has_unmarked=1;
                    //Warning(debug,"found unmarked edge");
                    //break;
                }
            }
            if (has_unmarked) continue;
            Warning(debug,"no unmarked edges");
        ...
        em_fini();
        Warning(debug,"%lld new %d/%d states handled",local_found,level_begin[level_count],my_states);
        ...
        if (mpi_me==0 && RTverbosity > 1){
            Warning(info,"tau level %d has %lld states",level_count,global_found);
        }
    }
    if (level_begin[level_count]==my_states) {
        if(mpi_me==0) Warning(info,"The LTS has %d tau levels",level_count);
    } else {
        Fatal(1,error,"the LTS is not tau cycle free");
    }
    ...
    if(mpi_me==0){
        SCCstopTimer(timer);
        SCClogTimer(info,timer,"building the tau levels took");
...
sig_id_t *sigmin_set_strong_tcf(event_queue_t queue,seg_lts_t lts,uint32_t tau_arg){
    ...
    if(mpi_me==0){
        timer=SCCcreateTimer();
        Warning(debug,"starting inductive reduction");
        SCCstartTimer(timer);
    }
    event_barrier_wait(barrier);
    int iter=0;
    ...
    while(oldcount!=newcount){
        ...
        MPI_Barrier(MPI_COMM_WORLD);
        if (mpi_me==0) Warning(debug,"main loop");
        MPI_Barrier(MPI_COMM_WORLD);
        iter++;
        oldcount=newcount;
        SetClear(SET_TAG_UNDEF);
        ...
        for(int L=0;L<level_count;L++){
            event_barrier_wait(barrier);
            //if (mpi_me==0) Warning(debug,"processing level %d",L);
    ...
    if(mpi_me==0){
        SCCstopTimer(timer);
        SCClogTimer(info,timer,"cycle free iterations took");
        SCCdeleteTimer(timer);
    }
    if ((uint32_t)mpi_me==0) {
        Warning(info,"reduced state space has %d states",newcount);
...
sig_id_t *sigmin_set_branching_tcf(event_queue_t queue,seg_lts_t lts,uint32_t tau_arg){
    ...
    while(oldcount!=newcount){
        ...
        for(int L=0;L<level_count;L++){
            ...
            Warning(debug,"propagating signatures");
            sig_init();
            for(uint32_t k=level_begin[L];k<level_begin[L+1];k++){
                int i=level_member[k];
                for(uint32_t j=in_begin[i];j<in_begin[i+1];j++){
