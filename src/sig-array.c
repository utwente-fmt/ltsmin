...
sig_event_t SigArrayNext(sig_array_t sa){
    switch(sa->state){
        ...
        case SEND_ID:
            ...
            sa->state=WAIT_ID;
            Warning(debug,"transition to WAIT");
        case WAIT_ID:
            TQwhile(SLTSgetQueue(sa->lts),&sa->id_count);
            sa->count=0;
            sa->state=COMPUTE_ID;
            Warning(debug,"transition to COMPUTE");
        case COMPUTE_ID:
            if(sa->count<sa->my_states){
                sig_event_t event;
            ...
            if(sa->count==sa->my_states){
                sa->state=IDLE;
                Warning(debug,"transition to IDLE");
...
void SigArrayStartRound(sig_array_t sa){
	Warning(debug,"id_count is %d",sa->id_count);
    ...
    if (sa->dest_id ==NULL) {
        ...
        for(int i=0;i<segment_count;i++){
            Warning(debug,"count %d edge to %d",count[i],i);
...
/*
    ...
    for(int i=0;i<K;i++){
        ...
        struct sig_entry e;
        //Warning(debug,"add [%d.%d]==%lld to sig of %d",edge[1],edge[2],sa->dest_id[edge[1]][edge[2]],state);
...
*/
    ...
    //Warning(debug,"state %d has %d edges",state,N);
